...
//console.table(wire1Coordinates);

for (let i = 0; i < path2.length; i++) {
    if (path2[i][0] === "R") {
        const value = parseInt(path2[i].slice(1));
        for (let j = 1; j <= value; j++) {
            wire2Coordinates.push({ x: wire2Coordinates[wire2Coordinates.length - 1].x + 1, y : wire2Coordinates[wire2Coordinates.length - 1].y});
        }
    } else if (path2[i][0] === "U") {
        const value = parseInt(path2[i].slice(1));
        for (let j = 1; j <= value; j++) {
           wire2Coordinates.push({ x: wire2Coordinates[wire2Coordinates.length - 1].x, y : wire2Coordinates[wire2Coordinates.length - 1].y + 1});
        }
    } else if (path2[i][0] === "L") {
        const value = parseInt(path2[i].slice(1));
        for (let j = 1; j <= value; j++) {
...
//console.table(wire2Coordinates);
const intersections = [];
    ...
    return function(current) {
      return otherArray.filter(function(other){
        if (other.x === current.x && other.y === current.y) {
            intersections.push([otherArray.indexOf(other), wire1Coordinates.indexOf(current)]);
        }
...
const result = wire1Coordinates
                    .filter(comparer(wire2Coordinates))
...
const result2 = intersections
                    .filter(arr => arr[0] !== 0 && arr[1] !== 0)
                    .map(arr => Math.abs(arr[0]) + Math.abs(arr[1]))
                    .sort((a,b) => a > b ? 1 : -1)[0];
